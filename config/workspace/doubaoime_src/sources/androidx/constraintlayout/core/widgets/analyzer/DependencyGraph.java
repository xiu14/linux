...
public class DependencyGraph {
    ...
    private String generateChainDisplayGraph(ChainRun chainRun, String str) {
        ...
        StringBuilder R = a.R("subgraph ", "cluster_");
        ...
        while (it2.hasNext()) {
            ...
            R.append(next.widget.getDebugName());
            if (i == 0) {
                R.append("_HORIZONTAL");
            } else {
                R.append("_VERTICAL");
            }
            R.append(";\n");
    ...
    private String generateDisplayNode(DependencyNode dependencyNode, boolean z, String str) {
        ...
        for (DependencyNode dependencyNode2 : dependencyNode.targets) {
            StringBuilder M = a.M("\n");
            M.append(dependencyNode.name());
            StringBuilder S = a.S(M.toString(), " -> ");
            S.append(dependencyNode2.name());
            String sb2 = S.toString();
            if (dependencyNode.margin > 0 || z || (dependencyNode.run instanceof HelperReferences)) {
                String s = a.s(sb2, "[");
                if (dependencyNode.margin > 0) {
                    s = a.D(a.S(s, "label=\""), dependencyNode.margin, "\"");
                    if (z) {
                        s = a.s(s, Constants.ACCEPT_TIME_SEPARATOR_SP);
                    }
                }
                if (z) {
                    s = a.s(s, " style=dashed ");
                }
                if (dependencyNode.run instanceof HelperReferences) {
                    s = a.s(s, " style=bold,color=gray ");
                }
                sb2 = a.s(s, "]");
            }
            sb.append(sb2 + "\n");
    ...
    private String nodeDefinition(WidgetRun widgetRun) {
        ...
        a.K0(sb, " [shape=none, label=<", "<TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"2\">", "  <TR>");
        if (z) {
            sb.append("    <TD ");
