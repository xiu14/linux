...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        float f2 = width / 2;
        float distanceInfluenceForSnapDuration = (distanceInfluenceForSnapDuration(Math.min(1.0f, Math.abs(i) / width)) * f2) + f2;
        int abs = Math.abs(i2);
    ...
    private int computeSettleDuration(View view, int i, int i2, int i3, int i4) {
        ...
        return (int) ((computeAxisDuration(i2, clampMag2, this.mCallback.getViewVerticalDragRange(view)) * (f4 / f5)) + (computeAxisDuration(i, clampMag, this.mCallback.getViewHorizontalDragRange(view)) * f6));
    }
    ...
    public void captureChildView(@NonNull View view, int i) {
        if (view.getParent() != this.mParentView) {
            StringBuilder M = a.M("captureChildView: parameter must be a descendant of the ViewDragHelper's tracked parent view (");
