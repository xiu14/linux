...
public abstract class MediaBrowserServiceCompat extends Service {
                ...
                public void run() {
                    if (!MediaBrowserServiceImplApi21.this.mRootExtrasList.isEmpty()) {
                        ...
                        if (b != null) {
                            ...
                            while (it2.hasNext()) {
                                BundleCompat.putBinder(it2.next(), MediaBrowserProtocol.EXTRA_SESSION_BINDER, b.asBinder());
                            }
                        }
                        MediaBrowserServiceImplApi21.this.mRootExtrasList.clear();
                    }
                    MediaBrowserServiceCompatApi21.setSessionToken(MediaBrowserServiceImplApi21.this.mServiceObj, token.c());
                }
            });
        }
                ...
                public void run() {
                    ...
                    while (it2.hasNext()) {
                        ...
                        try {
                            ...
                            StringBuilder M = a.M("Connection for ");
                            M.append(next.pkg);
                            M.append(" is no longer valid.");
                            Log.w(MediaBrowserServiceCompat.TAG, M.toString());
                            it2.remove();
                        }
                    }
                }
    ...
    public static class Result<T> {
        ...
        public void detach() {
            if (this.mDetachCalled) {
                StringBuilder M = a.M("detach() called when detach() had already been called for: ");
                M.append(this.mDebug);
                throw new IllegalStateException(M.toString());
            }
            if (this.mSendResultCalled) {
                StringBuilder M2 = a.M("detach() called when sendResult() had already been called for: ");
            ...
            if (!this.mSendErrorCalled) {
                ...
                StringBuilder M3 = a.M("detach() called when sendError() had already been called for: ");
        ...
        void onErrorSent(Bundle bundle) {
            StringBuilder M = a.M("It is not supported to send an error for ");
            M.append(this.mDebug);
            throw new UnsupportedOperationException(M.toString());
        }

        void onProgressUpdateSent(Bundle bundle) {
            StringBuilder M = a.M("It is not supported to send an interim update for ");
            M.append(this.mDebug);
            throw new UnsupportedOperationException(M.toString());
        }
        ...
        public void sendError(Bundle bundle) {
            if (this.mSendResultCalled || this.mSendErrorCalled) {
                StringBuilder M = a.M("sendError() called when either sendResult() or sendError() had already been called for: ");
                M.append(this.mDebug);
                throw new IllegalStateException(M.toString());
            }
        ...
        public void sendProgressUpdate(Bundle bundle) {
            if (this.mSendResultCalled || this.mSendErrorCalled) {
                StringBuilder M = a.M("sendProgressUpdate() called when either sendResult() or sendError() had already been called for: ");
                M.append(this.mDebug);
                throw new IllegalStateException(M.toString());
            }
        ...
        public void sendResult(T t) {
            if (this.mSendResultCalled || this.mSendErrorCalled) {
                StringBuilder M = a.M("sendResult() called when either sendResult() or sendError() had already been called for: ");
                M.append(this.mDebug);
                throw new IllegalStateException(M.toString());
            }
        ...
        public void addSubscription(final String str, final IBinder iBinder, final Bundle bundle, final ServiceCallbacks serviceCallbacks) {
            MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() { // from class: androidx.media.MediaBrowserServiceCompat.ServiceBinderImpl.3
                ...
                public void run() {
                    ...
                    StringBuilder M = a.M("addSubscription for callback that isn't registered id=");
                    M.append(str);
                    Log.w(MediaBrowserServiceCompat.TAG, M.toString());
                }
            });
            ...
            if (MediaBrowserServiceCompat.this.isValidPackage(str, i2)) {
                MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() { // from class: androidx.media.MediaBrowserServiceCompat.ServiceBinderImpl.1
                    ...
                    public void run() {
                        ...
                        if (onGetRoot == null) {
                            StringBuilder M = a.M("No root for client ");
                            M.append(str);
                            M.append(" from service ");
                            M.append(getClass().getName());
                            Log.i(MediaBrowserServiceCompat.TAG, M.toString());
                            try {
                                serviceCallbacks.onConnectFailed();
                                return;
                            } catch (RemoteException unused) {
                                StringBuilder M2 = a.M("Calling onConnectFailed() failed. Ignoring. pkg=");
                                M2.append(str);
                                Log.w(MediaBrowserServiceCompat.TAG, M2.toString());
                                return;
                            }
                        }
                        try {
                            ...
                            StringBuilder M3 = a.M("Calling onConnect() failed. Dropping client. pkg=");
                            M3.append(str);
                            Log.w(MediaBrowserServiceCompat.TAG, M3.toString());
                            MediaBrowserServiceCompat.this.mConnections.remove(asBinder);
                        }
                    }
                });
        ...
        public void getMediaItem(final String str, final ResultReceiver resultReceiver, final ServiceCallbacks serviceCallbacks) {
            ...
            MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() { // from class: androidx.media.MediaBrowserServiceCompat.ServiceBinderImpl.5
                ...
                public void run() {
                    ...
                    StringBuilder M = a.M("getMediaItem for callback that isn't registered id=");
                    M.append(str);
                    Log.w(MediaBrowserServiceCompat.TAG, M.toString());
                }
            });
        ...
        public void removeSubscription(final String str, final IBinder iBinder, final ServiceCallbacks serviceCallbacks) {
            MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() { // from class: androidx.media.MediaBrowserServiceCompat.ServiceBinderImpl.4
                ...
                public void run() {
                    ConnectionRecord connectionRecord = MediaBrowserServiceCompat.this.mConnections.get(serviceCallbacks.asBinder());
                    if (connectionRecord == null) {
                        StringBuilder M = a.M("removeSubscription for callback that isn't registered id=");
                        M.append(str);
                        Log.w(MediaBrowserServiceCompat.TAG, M.toString());
                    } else {
                        ...
                        StringBuilder M2 = a.M("removeSubscription called for ");
                        M2.append(str);
        ...
        public void search(final String str, final Bundle bundle, final ResultReceiver resultReceiver, final ServiceCallbacks serviceCallbacks) {
            ...
            MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() { // from class: androidx.media.MediaBrowserServiceCompat.ServiceBinderImpl.8
                ...
                public void run() {
                    ...
                    StringBuilder M = a.M("search for callback that isn't registered query=");
                    M.append(str);
                    Log.w(MediaBrowserServiceCompat.TAG, M.toString());
                }
            });
        ...
        public void sendCustomAction(final String str, final Bundle bundle, final ResultReceiver resultReceiver, final ServiceCallbacks serviceCallbacks) {
            ...
            MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() { // from class: androidx.media.MediaBrowserServiceCompat.ServiceBinderImpl.9
                ...
                public void run() {
                    ...
                    StringBuilder M = a.M("sendCustomAction for callback that isn't registered action=");
                    M.append(str);
                    M.append(", extras=");
                    M.append(bundle);
                    Log.w(MediaBrowserServiceCompat.TAG, M.toString());
    ...
    void performLoadChildren(final String str, final ConnectionRecord connectionRecord, final Bundle bundle, final Bundle bundle2) {
            ...
            public void onResultSent(List<MediaBrowserCompat.MediaItem> list) {
                if (MediaBrowserServiceCompat.this.mConnections.get(connectionRecord.callbacks.asBinder()) != connectionRecord) {
                    if (MediaBrowserServiceCompat.DEBUG) {
                        StringBuilder M = a.M("Not sending onLoadChildren result for connection that has been disconnected. pkg=");
                        M.append(connectionRecord.pkg);
                        M.append(" id=");
                        M.append(str);
                        Log.d(MediaBrowserServiceCompat.TAG, M.toString());
                        return;
                    }
                    return;
                }
                ...
                try {
                    ...
                    StringBuilder M2 = a.M("Calling onLoadChildren() failed for id=");
                    M2.append(str);
                    M2.append(" package=");
        ...
        this.mCurConnection = null;
        if (!result.isDone()) {
            throw new IllegalStateException(a.K(a.M("onLoadChildren must call detach() or sendResult() before returning for package="), connectionRecord.pkg, " id=", str));
        }
    }
    ...
    void performLoadItem(String str, ConnectionRecord connectionRecord, final ResultReceiver resultReceiver) {
        ...
        this.mCurConnection = null;
        if (!result.isDone()) {
            throw new IllegalStateException(a.s("onLoadItem must call detach() or sendResult() before returning for id=", str));
        }
    }
    ...
    void performSearch(String str, Bundle bundle, ConnectionRecord connectionRecord, final ResultReceiver resultReceiver) {
        ...
        this.mCurConnection = null;
        if (!result.isDone()) {
            throw new IllegalStateException(a.s("onSearch must call detach() or sendResult() before returning for query=", str));
        }
    }
