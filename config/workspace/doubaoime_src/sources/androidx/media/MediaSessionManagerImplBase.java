...
class MediaSessionManagerImplBase implements MediaSessionManager.MediaSessionManagerImpl {
    ...
    public boolean isTrustedForMediaControl(@NonNull MediaSessionManager.RemoteUserInfoImpl remoteUserInfoImpl) {
        try {
            ...
            if (DEBUG) {
                StringBuilder M = a.M("Package name ");
                M.append(remoteUserInfoImpl.getPackageName());
                M.append(" doesn't match with the uid ");
                a.z0(M, remoteUserInfoImpl.getUid(), TAG);
            }
            return false;
        } catch (PackageManager.NameNotFoundException unused) {
            if (DEBUG) {
                StringBuilder M2 = a.M("Package ");
                M2.append(remoteUserInfoImpl.getPackageName());
                M2.append(" doesn't exist");
