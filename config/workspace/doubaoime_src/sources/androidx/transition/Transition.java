...
public abstract class Transition implements Cloneable {
    ...
    private static int[] parseMatchOrder(String str) {
        ...
        while (stringTokenizer.hasMoreTokens()) {
            ...
            if (MATCH_ID_STR.equalsIgnoreCase(trim)) {
                ...
                if (!trim.isEmpty()) {
                    throw new InflateException(a.t("Unknown match type in matchOrder: '", trim, "'"));
                }
    ...
    String toString(String str) {
        ...
        if (this.mDuration != -1) {
            sb = a.F(a.S(sb, "dur("), this.mDuration, ") ");
        }
        if (this.mStartDelay != -1) {
            sb = a.F(a.S(sb, "dly("), this.mStartDelay, ") ");
        }
        ...
        String s = a.s(sb, "tgts(");
        if (this.mTargetIds.size() > 0) {
            for (int i = 0; i < this.mTargetIds.size(); i++) {
                if (i > 0) {
                    s = a.s(s, ", ");
                }
                StringBuilder M2 = a.M(s);
                M2.append(this.mTargetIds.get(i));
                s = M2.toString();
            }
        }
        if (this.mTargets.size() > 0) {
            for (int i2 = 0; i2 < this.mTargets.size(); i2++) {
                ...
                StringBuilder M3 = a.M(s);
                M3.append(this.mTargets.get(i2));
                s = M3.toString();
            }
        }
        return a.s(s, ")");
