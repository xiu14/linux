...
#region Logic
  ...
  private void DrawSocialPageOptions()
  {
    ...
    for (int i = _socialPage.slotPosition; i < _socialPage.slotPosition + 5 && i < _friendNames.Count; ++i)
    {
      ...
      MobileTapTooltip.Register(checkbox.bounds, I18n.TrackOnMap());
  ...
  private static void DrawNPC(NPC character, List<string> namesToShow)
  {
    ...
    var headShotBounds = new Rectangle(
      (int)offsetLocation.X,
      (int)offsetLocation.Y,
      (int)(headShot.Width * headShotScale),
      (int)(headShot.Height * headShotScale)
    );
    MobileTapTooltip.Register(headShotBounds, character.displayName);
