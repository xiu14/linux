...
internal class ShowCropAndBarrelTime : IDisposable
{
  ...
  private const int MobileTooltipUpdateTicks = 45;
  ...
  private readonly PerScreen<Vector2?> _mobileSelectedTile = new();
  private readonly PerScreen<int> _mobileSelectedTileTicks = new();
  ...
  {
    if (!Context.IsWorldReady || Game1.activeClickableMenu != null || Game1.currentLocation is null)
    {
      return;
    }
    ...
    _mobileSelectedTile.Value = e.Cursor.GrabTile;
    _mobileSelectedTileTicks.Value = MobileTooltipUpdateTicks;
  }
  ...
  private void OnUpdateTicked(object? sender, UpdateTickedEventArgs e)
  {
    ...
    if (_mobileSelectedTileTicks.Value > 0)
    {
      _mobileSelectedTileTicks.Value--;
    }

    Vector2 tile = _mobileSelectedTileTicks.Value > 0 && _mobileSelectedTile.Value.HasValue
      ? _mobileSelectedTile.Value.Value
  ...
  private void OnRenderingHud(object? sender, RenderingHudEventArgs e)
  {
    ...
    if (_mobileSelectedTileTicks.Value > 0 || Game1.options.gamepadControls && Game1.timerUntilMouseFade <= 0)
    {
