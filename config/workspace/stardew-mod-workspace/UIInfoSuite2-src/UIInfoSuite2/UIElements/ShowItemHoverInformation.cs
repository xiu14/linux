...
{
  private const int MobileTooltipTicks = 180;
  ...
  private readonly PerScreen<Item?> _mobileSelectedItem = new();
  private readonly PerScreen<int> _mobileSelectedItemTicks = new();
  ...
  private void OnButtonPressed(object? sender, ButtonPressedEventArgs e)
  {
    ...
    Item? item = Tools.GetHoveredItem();
    if (item is null)
    {
      return;
    }

    _mobileSelectedItem.Value = item;
    _mobileSelectedItemTicks.Value = MobileTooltipTicks;
  }

  private void OnUpdateTicked(object? sender, UpdateTickedEventArgs e)
  {
    if (_mobileSelectedItemTicks.Value > 0)
    {
      _mobileSelectedItemTicks.Value--;
    }
  }
  ...
  {
    Item? hoverItem = GetTooltipItem();
    if (hoverItem != null && !(hoverItem is MeleeWeapon weapon && weapon.isScythe()) && hoverItem is not FishingRod)
    {
      var hoveredObject = hoverItem as Object;

      int itemPrice = Tools.GetSellToStorePrice(hoverItem);

      var stackPrice = 0;
      if (itemPrice > 0 && hoverItem.Stack > 1)
      {
        stackPrice = itemPrice * hoverItem.Stack;
      }

      int cropPrice = Tools.GetHarvestPrice(hoverItem);

      bool notDonatedYet = _libraryMuseum.isItemSuitableForDonation(hoverItem);
  ...
  private Item? GetTooltipItem()
  {
    return _hoverItem.Value ?? (_mobileSelectedItemTicks.Value > 0 ? _mobileSelectedItem.Value : null);
  }
